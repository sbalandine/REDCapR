#' @title Import instruments-event mapping of a REDCap project
#'
#' @description
#' Import instruments-event mapping (*i.e.*, data dictionary)
...
#' outcome.
#' * `item_count`: Number of fields imported.
...
#'   readr::read_csv(
#'     file = system.file(
#'       "test-data/project-simple/simple-instruments-event mapping.csv",
#'       package = "REDCapR"
#'     ),
...
#' REDCapR::redcap_events_instrument_write(
#'   ds          = ds_to_write,
#'   redcap_uri  = uri,
#'   token       = token
#' )
...
#'
#' @export
redcap_events_instrument_write <- function(
  ds,
  redcap_uri,
  token,
  verbose         = TRUE,
  ...
  if (kernel$success) {
    item_count           <- as.integer(kernel$raw_text)
    outcome_message       <- sprintf(
      "%s instrument-event mapping were written to the REDCap dictionary in %0.1f seconds.",
      format(item_count, big.mark = ",", scientific = FALSE, trim = TRUE),
    ...
    item_count            <- 0L
    outcome_message        <- sprintf(
      "The REDCapR write/import instruments-event mapping operation was not successful.  The error message was:\n%s",
      kernel$raw_text
    )
  }
