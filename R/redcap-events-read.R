#' @title Export the events of a REDCap project
#'
#' @description Export the events (as a data dictionary) of a REDCap project
...
#' REDCapR::redcap_events_read(redcap_uri=uri, token=token)
#' }

#' @export
redcap_events_read <- function(
  redcap_uri,
  token,
  verbose           = TRUE,
  config_options    = NULL
  ...
  if (kernel$success) {
    ...
    col_types <-
      readr::cols(
        ...
        day_offset  = readr::col_integer(),
        offset_min  = readr::col_integer(),
        offset_max  = readr::col_integer(),
    ...
    if (exists("ds") & inherits(ds, "data.frame")) {
      outcome_message <- sprintf(
        "The data dictionary describing %s events was read from REDCap in %0.1f seconds.  The http status code was %i.",
      ...
      ds                <- data.frame()
      outcome_message   <- sprintf(
        "The REDCap events export failed.  The http status code was %i.  The 'raw_text' returned was '%s'.",
        kernel$status_code,
        kernel$raw_text
      )
    }       # nocov end
    ...
    outcome_message     <- sprintf(
      "The REDCapR events export operation was not successful.  The error message was:\n%s",
      kernel$raw_text
    )
  }
