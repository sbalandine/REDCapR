#' @title Export the instrument-event mapping of a REDCap project
#'
#' @description Export the instrument-event mapping (as a data dictionary) of a REDCap project
  ...
  if (kernel$success) {
    ...
    if (exists("ds") & inherits(ds, "data.frame")) {
      outcome_message <- sprintf(
        "The data dictionary describing %s instrument-event mapping was read from REDCap in %0.1f seconds.  The http status code was %i.",
      ...
      ds                <- data.frame()
      outcome_message   <- sprintf(
        "The REDCap instrument-event mapping export failed.  The http status code was %i.  The 'raw_text' returned was '%s'.",
        kernel$status_code,
        kernel$raw_text
      )
    }       # nocov end
    ...
    outcome_message     <- sprintf(
      "The REDCapR instrument-event mapping export operation was not successful.  The error message was:\n%s",
      kernel$raw_text
    )
  }
