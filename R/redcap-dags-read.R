#' @title Export the dags of a REDCap project
#'
#' @description Export the dags (as a data dictionary) of a REDCap project
...
#' REDCapR::redcap_dags_read(redcap_uri=uri, token=token)
#' }

#' @export
redcap_dags_read <- function(
  redcap_uri,
  token,
  verbose           = TRUE,
  config_options    = NULL
  ...
  if (kernel$success) {
    ...
    col_types <-
      readr::cols(
        data_access_group_name  = readr::col_character(),
        unique_group_name  = readr::col_character(),
    ...
    if (exists("ds") & inherits(ds, "data.frame")) {
      outcome_message <- sprintf(
        "The data dictionary describing %s data access group was read from REDCap in %0.1f seconds.  The http status code was %i.",
      ...
      ds                <- data.frame()
      outcome_message   <- sprintf(
        "The REDCap dags export failed.  The http status code was %i.  The 'raw_text' returned was '%s'.",
        kernel$status_code,
        kernel$raw_text
      )
    }       # nocov end
    ...
    outcome_message     <- sprintf(
      "The REDCapR dags export operation was not successful.  The error message was:\n%s",
      kernel$raw_text
    )
  }
