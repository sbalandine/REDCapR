#' @title Export the instrument of a REDCap project
#'
#' @description Export the instrument (as a data dictionary) of a REDCap project
...
#' REDCapR::redcap_instruments_read(redcap_uri=uri, token=token)
#' }

#' @export
redcap_instruments_read <- function(
  redcap_uri,
  token,
  verbose           = TRUE,
  config_options    = NULL
  ...
  if (kernel$success) {
    ...
    if (exists("ds") & inherits(ds, "data.frame")) {
      outcome_message <- sprintf(
        "The data dictionary describing %s instrument was read from REDCap in %0.1f seconds.  The http status code was %i.",
      ...
      ds                <- data.frame()
      outcome_message   <- sprintf(
        "The REDCap instrument export failed.  The http status code was %i.  The 'raw_text' returned was '%s'.",
        kernel$status_code,
        kernel$raw_text
      )
    }       # nocov end
    ...
    outcome_message     <- sprintf(
      "The REDCapR instrument export operation was not successful.  The error message was:\n%s",
      kernel$raw_text
    )
  }
